...
class NodeTypeDataset(data.Dataset):
    def __init__(self, env, node_type, state, pred_state, node_freq_mult,
     ...
     # 该类在初始化时需要传入环境、节点类型、状态、预测状态、节点频率乘数、场景频率乘数、超参数、是否增强和其他关键字参数
        self.env = env
        self.state = state
        self.pred_state = pred_state
        self.hyperparams = hyperparams
    ...
    def index_env(self, node_freq_mult, scene_freq_mult, **kwargs):
        ...
        for scene in self.env.scenes:
            ...
            for t, nodes in present_node_dict.items():
                ...
                # 将 (场景, 时间步骤, 节点) 这样的元组加入到索引列表中
                # 如果启用了场景频率乘数 scene_freq_mult，那么会将此元组重复加入场景的 frequency_multiplier 次
                # 如果启用了节点频率乘数 node_freq_mult，那么会将此元组重复加入节点的 frequency_multiplier 次
        return index
